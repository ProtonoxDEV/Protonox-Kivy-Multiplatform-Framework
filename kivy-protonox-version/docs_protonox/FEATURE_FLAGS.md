...
| --- | --- | --- |
| `KIVY_PROTONOX` / `PROTONOX_KIVY` | Requests Protonox safe mode without code changes. | unset |
...
| `PROTONOX_DEVICE_LAYER` | Enable modern Android device helpers (CameraX/AudioRecord/SAF). | `0` |
...
- `enable()` or `KIVY_PROTONOX=1`/`PROTONOX_KIVY=1` → sets `PROTONOX_COMPAT_MODE=1` only.
