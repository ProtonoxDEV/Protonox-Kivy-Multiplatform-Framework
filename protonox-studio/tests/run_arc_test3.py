...
with sync_playwright() as p:
    ...
    coords = el.evaluate(
        "e => { const r = e.getBoundingClientRect(); return {x: r.left + r.width/2, y: r.top + r.height/2, w: r.width, h: r.height}; }")
    print('coords=', coords)
    ...
    under = page.evaluate('(p) => { const el = document.elementFromPoint(p.x, p.y); return el ? {tag: el.tagName, id: el.id || null, cls: el.className ? el.className.toString().slice(0,200) : null} : null; }', {
                          "x": tx+20, "y": ty+20})
    ...
    if before_parent == after_parent:
        ...
        try:
            ...
            after_parent = el.evaluate(
                "e => (e.parentElement ? (e.parentElement.id || e.parentElement.tagName) : null)")
            ...
            diag = page.evaluate(
                '''() => ({ helperType: typeof window.__protonox_test_reparent, foundEl: !!document.querySelector('[data-px-test="1"]'), foundDrop: !!document.querySelector('#__px_test_drop') })''')
